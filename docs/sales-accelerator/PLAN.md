...
- [x] License tier gating: `LicenseInfo.sales_accelerator` parsed from Keygen `metadata.salesAccelerator`; `useSalesAccelerator()` hook (true in dev/skip mode) gates the sidebar nav link + shows an upsell on `/assistant` when not entitled. **Ops:** the Cloudflare Worker sets `metadata.salesAccelerator=true` when an order matches `POLAR_SALES_ACCELERATOR_PRODUCT_IDS` (the tier's Polar product id(s)) — set that var before selling the tier. Matches the app's existing frontend-gated posture; backend command-level enforcement is a hardening follow-up.
